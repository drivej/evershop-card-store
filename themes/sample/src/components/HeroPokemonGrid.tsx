...
const rotations = Array(200)
  .fill(0)
  .map(() => rand(-30, 30));

const offsets = Array(200)
  .fill(0)
  .map(() => ({
    top: rand(-30, 30),
    left: rand(-30, 30)
...
export default function HeroPokemonGrid() {
  ...
  return (
    <div ref={ref}>
      ...
      <section className='section-inset-shadow'>
        {cards.map((_, i) => (
          <a key={i} href='/'>
            <img
              key={`card${i}`}
        ...
        <div className='border-r-8 border-blue-500 z-50 relative backdrop-blur-xl bg-stone-800/50 text-white my-28 p-20 lg:w-[50%] inline-block rounded-r-3xl mb-[20%]'>
