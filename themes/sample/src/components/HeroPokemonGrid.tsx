...
function spiralCards(rect: { width: number; height: number }, config: CardConfig): Card[] {
  ...
  for (let i = 0; i < 200; i++) {
    ...
    if (visible) {
      cards.push({ id: cardId++, x, y, rotation, transitionDelay: i * 30 });
    }
...
function gridCards(rect: { width: number; height: number }, config: CardConfig): Card[] {
  const spacing = 10;
  ...
  for (let i = 0; i < totalCards; i++) {
    ...
    cards.push({ id: i, x, y, rotation: 0, transitionDelay: i * 30 });
  }
  return cards;
}

function messyCards(rect: { width: number; height: number }, config: CardConfig): Card[] {
  const cards: Card[] = [];
  for (let i = 0; i < 50; i++) {
    const y = rand(-config.height, rect.height);
    const x = rand(-config.width, rect.width);
    const rotation = rand(-30, 30);
    cards.push({ id: i, x, y, rotation, transitionDelay: i * 30 });
  }
  return cards;
}

function fanCards(rect: { width: number; height: number }, config: CardConfig): Card[] {
  const cards: Card[] = [];
  const centerX = rect.width / 2;
  ...
  for (let i = 0; i < totalCards; i++) {
    ...
    cards.push({ id: i, x, y, rotation });
  }
  return cards;
}

function stackCards(rect: { width: number; height: number }, config: CardConfig): Card[] {
  const cards: Card[] = [];
  ...
  for (let i = 0; i < 30; i++) {
    cards.push({ id: i, x, y, rotation: 0 });
  }
  return cards;
}
...
function offstageCards(rect: { width: number; height: number }, config: CardConfig): Card[] {
  const cards: Card[] = [];
  ...
  for (let i = 0; i < defaultCardConfig.totalCards; i++) {
    ...
    cards.push({ id: i, x, y, rotation: rand(-30, 30), transitionDelay: i * 3 });
  }
  return cards;
}
...
export default function HeroPokemonGrid() {
  ...
  const offstageLayout = useMemo(() => offstageCards({ width, height }, defaultCardConfig), [width, height]);
  const spiralLayout = useMemo(() => spiralCards({ width, height }, defaultCardConfig), [width, height]);
  const gridLayout = useMemo(() => gridCards({ width, height }, defaultCardConfig), [width, height]);
  ...
  const cards = useMemo(() => {
    ...
    for (let i = 0; i < 200; i++) {
      if (i < currentLayout.length) {
        combined.push({ ...currentLayout[i], id: i });
      } else {
        combined.push({ ...offstageLayout[i], id: i });
      }
    }
    return combined;
