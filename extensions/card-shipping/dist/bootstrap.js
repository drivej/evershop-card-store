 ...
 */ async function getCardItemsFromCart(cart) {
    ...
    try {
        ...
        const query = select().from('cart_item').leftJoin('product', 'product.product_id', '=', 'cart_item.product_id').leftJoin('product_attribute_value_index', 'product_attribute_value_index.product_id', '=', 'product.product_id').leftJoin('attribute', 'attribute.attribute_id', '=', 'product_attribute_value_index.attribute_id').where('cart_item.cart_id', '=', cart.cart_id);
